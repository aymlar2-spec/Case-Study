...
st.caption(
    "Le score global est un indicateur de synthèse. L'analyse décisionnelle repose principalement sur les sous-scores par dimension et la typologie de l'établissement."
)
...
with left:
    ...
    values = [
        round(row["score_d1_resultats"], 1),
        round(row["score_d2_disponibilite"], 1),
        round(row["score_d3_mobilisation"], 1),
        round(row["score_d4_enseignants"], 1),
        round(row["score_d5_historique"], 1),
        round(row["score_d6_couverture"], 1),
        round(row["score_d7_contraintes"], 1)
    ]
...
with right:
    ...
    for dimension, infos in row["profil_detaille"].items():
        ...
        st.caption(f"{infos['score']:.1f}/100 • {infos['niveau']}")
